...
function Ensure-Current($path) {
    $JObject = [Newtonsoft.Json.JsonConvert]::DeserializeObject((Get-Content $path))
    Ensure-Node -root $JObject -names 'Stacks.Data.EntityFramework.ConnectionString'
	Ensure-Node -root $JObject -names 'Stacks.Data.EntityFramework.AutoAddSearchResults'
    
    if ($JObject -ne $null) {
		"log  : Ensuring Entity Framework settings "
        [Newtonsoft.Json.JsonConvert]::SerializeObject($JObject) > $path
    }
}
